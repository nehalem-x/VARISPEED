...
- em atualizações incrementais, posições preservadas de `root` e categorias alimentam `spawnAnchors`. `_categorySpawnAngle()` coloca cada categoria nova no maior intervalo angular e usa a distância real aos nós existentes como desempate; não voltar a calcular o spawn incremental apenas pelo índice da lista;
...
- wheel/trackpad acumula `deltaY`; câmera, física e pintura são consumidas por um único `requestAnimationFrame`. Não recriar um loop separado de câmera nem medir host e painéis para cada evento bruto;
...
- ligações pertencem a `.graph-engine-links-canvas`, atrás do SVG interativo `.graph-engine-nodes-svg`. `_renderLinks()` transforma os pontos físicos em coordenadas de tela, rejeita segmentos inteiramente fora do viewport e agrupa traços pela mesma cor/opacidade/tracejado; `maxLinkPixelRatio: 2` limita o backing store sem perder nitidez no DPR normal. Não recriar `<line>` por ligação;
- `_renderPositions()` interpola o último passo físico, move os nós antes de desenhar o Canvas e registra `_graphRenderedX/Y` como snapshot estritamente visual. As conexões projetam exatamente esse snapshot; um nó com `fx/fy` é imediato e nunca recebe atraso de interpolação;
- `applyCamera()` afeta apenas o SVG dos nós: até `vectorCameraEnterScale: 1.16`, escreve a transformação composta em `world`; no zoom próximo, normaliza o mundo e serializa a câmera em `viewBox`. `vectorCameraExitScale: 1.04` fornece histerese. O Canvas usa sempre `camera.x/y/scale` diretamente; preservar o cache `_graphCamera`/`_graphViewBox`;
- `cameraSpringMs: 150` controla uma mola criticamente amortecida analítica. Ela converge sem ultrapassar o alvo e substitui tanto o salto em dois quadros quanto o hold fixo de `72 ms`;
- a física usa `physicsHz: 60` enquanto ativa e `restingPhysicsHz: 30` quando `alpha`, arraste e recuperação permitem repouso. Forças, retenção de velocidade e decay são normalizados pelo passo; a interpolação visual preserva fluidez no refresh do monitor;
- o snapshot de desempenho inclui latência/intervalo/custo/resposta do zoom, atraso de entrada, quadros perdidos estimados por múltiplos do refresh e long tasks; esses campos alimentam Configurações → Avançado → Diagnóstico e são somente observacionais;
- `startPerformanceCapture()` agrega intervalos reais de `requestAnimationFrame` e custos de física, render e câmera. O relatório usa média/p95/máximo, estima o orçamento pela mediana, classifica quadros acima de `1,5×`, conta múltiplos e observa Long Tasks quando a API existe;
- Configurações → Avançado → Diagnóstico controla essa captura por `MediaLibrary.measurePerformance()`, fecha o painel enquanto o usuário exercita o grafo e reabre com um relatório copiável. A captura não pode persistir dados, criar nós sintéticos ou alterar forças;
- `applyCamera()` conserva a última serialização em cada plano e não repete `style.transform` quando a transformação é idêntica;
- `_renderPositions()` arredonda apenas a serialização visual a centésimos de pixel, sem arredondar `node.x/y/vx/vy`;
