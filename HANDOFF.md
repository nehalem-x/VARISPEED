...
- ligações e nós pertencem a SVGs irmãos (`.graph-engine-links-svg` e `.graph-engine-nodes-svg`) com `viewBox` sincronizado. `linkWorld` e `world` recebem exatamente a mesma câmera; não voltar a unir linhas, textos e nós no mesmo plano de pintura;
...
- `applyCamera()` escreve `style.transform` em `linkWorld` e `world`, com `transform-box:view-box`, `transform-origin:0 0` e promoção ao compositor; manter uma única escrita por camada em cada frame e preservar o cache `_graphCamera`;
