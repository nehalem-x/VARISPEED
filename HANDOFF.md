...
- `_renderPositions()` desenha o Canvas e move cada nó por `style.transform`, permitindo composição independente. No hold da roda, somente escritas dos nós são consolidadas; o Canvas barato continua acompanhando a câmera para que as conexões nunca atrasem. Coordenadas físicas continuam intocadas;
