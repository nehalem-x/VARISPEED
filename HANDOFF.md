...
- ligações e nós pertencem a SVGs irmãos (`.graph-engine-links-svg` e `.graph-engine-nodes-svg`) recortados no viewport e isolados por `contain: layout paint`. Não voltar a unir linhas, textos e nós no mesmo plano de pintura nem restaurar `overflow: visible`;
- `_renderPositions()` move cada nó por `style.transform`, permitindo composição independente, e mantém os atributos `x1/y1/x2/y2` somente nas ligações. Coordenadas físicas continuam intocadas;
- `applyCamera()` é adaptativo: até `vectorCameraEnterScale: 1.16`, escreve a mesma transformação composta em `linkWorld` e `world`; no zoom próximo, normaliza ambos os mundos e serializa a câmera em `viewBox` idêntico nos dois SVGs. `vectorCameraExitScale: 1.04` fornece histerese. Isso evita a grande superfície rasterizada criada ao ampliar o SVG inteiro, sem mudar coordenadas, ponteiro ou física; preservar os caches `_graphCamera`/`_graphViewBox` e a sincronização entre planos;
