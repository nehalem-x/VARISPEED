...
- `_renderPositions()` move os nós antes de desenhar o Canvas e registra `_graphRenderedX/Y` como snapshot estritamente visual. No hold da roda, somente escritas dos nós são consolidadas; as conexões continuam acompanhando a câmera, mas projetam esse mesmo snapshot para nunca antecipar os círculos. Coordenadas físicas continuam intocadas;
