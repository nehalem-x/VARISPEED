...
- títulos longos não devem voltar a ser truncados: `Motion.marquee()` cuida do cabeçalho, detalhe da Biblioteca e `#sName` em Fonte por overflow real; `GraphEngine` usa `nodeLabelMaxWidth` para recortar e animar o texto diretamente no Canvas;
- o marquee não participa da geometria da física. Um `.graph-engine-label-hit` transparente preserva a área clicável no SVG, nomes curtos não recebem animação e `reduceMotion` desativa o movimento;
...
- ligações, títulos e contagens pertencem a `.graph-engine-links-canvas`, atrás do SVG interativo `.graph-engine-nodes-svg`. `_renderLinks()` transforma os pontos físicos em coordenadas de tela, rejeita segmentos inteiramente fora do viewport, agrupa traços e chama `_renderLabels()`; `maxLinkPixelRatio: 2` limita o backing store. Não recriar `<line>`, `<text>`, `<clipPath>` ou `<animate>` por nó;
- `_renderPositions()` desenha o Canvas e move somente os grupos de hit/círculos por `style.transform`. No hold da roda, os pontos `_graphRenderedX/Y` mantêm Canvas e SVG alinhados até a próxima escrita integral. Coordenadas físicas continuam intocadas;
...
- `wheelRenderHoldMs: 72` consolida somente `_renderPositions()` durante o burst da roda. `_tick()` continua executando integralmente física, colisão, movimento e decay; arraste de nó ignora o hold e o primeiro frame posterior redesenha a posição física atual;
- o snapshot de desempenho inclui `zoomLatencyMs`, `zoomFrameMs`, `zoomCameraMs`, `zoomSettleMs` e `zoomDroppedFrames`; a captura também conta `programmaticFrames`, de modo que foco por clique e controles deixem de aparecer como “Zoom não exercitado”. Esses campos são apenas métricas e não interferem na física;
- `startPerformanceCapture()` é estritamente observacional: durante oito segundos agrega intervalos reais de `requestAnimationFrame` e custos já medidos de física, render e câmera. O relatório usa média/p95/máximo, estima o orçamento pela mediana dos frames e classifica como tardio apenas o quadro acima de `1,5×` esse orçamento, acomodando telas de 60/120/144 Hz sem mudar a simulação;
